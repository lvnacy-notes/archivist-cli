...
scene additions, character development, plot advancement, workflow updates, etc.

Scopes automatically to the current git repo (or submodule) root. Searches for
CHANGELOG_TEMPLATE.md recursively under ARCHIVE/ — shallowest match wins.
Output is written to the same directory the template lives in (ARCHIVE/CHANGELOG/).
"""
...
def _find_story_template(git_root: Path) -> Path:
    """
    Search recursively under ARCHIVE/ for CHANGELOG_TEMPLATE.md.
    Shallowest match wins. Expected path: ARCHIVE/CHANGELOG/CHANGELOG_TEMPLATE.md.
    """
    ...
    if not matches:
        print(
            ...
            f"       Rename your CHANGELOG.md template to CHANGELOG_TEMPLATE.md.",
            file=sys.stderr,
        )
        sys.exit(1)
...
### Key Discussions
[Summary of conversations that drove changes]
...
**Primary Commit SHA**: {commit_sha or "[fill in after commit]"}
**Commit Message**: [fill in before committing]
**Files in Commit**: [fill in before committing]
...
*Changelog auto-generated by archivist changelog story — fill in bracketed fields before committing.*
"""
...
def run(args: argparse.Namespace) -> None:
    ...
    template_path = _find_story_template(git_root)
    ...
    frontmatter = _build_story_frontmatter(
        template_fm, args.commit_sha,
        num_modified, num_added, num_archived,
        git_root,
    )
    body = _build_story_body(changes, args.commit_sha, git_root)
